...
export type Scope = Record<string, IdentFlags>;
            ...
            if (ex.object.type === "Identifier") {
                ...
                if (ident.isObjectPrototype || ex.object.name === "Object") {
                    if (
                        ...
                        !NON_MUTATING_OBJECT_PROTOTYPE_METHODS.has(
                            ex.property.name,
                        )
                    ) {
                        mergeFlags(flags, {
...
export function setDeepFlags(
    ...
    switch (ex.type) {
        ...
        case "ArrayPattern":
            return ex.elements.forEach((elem) =>
                setDeepFlags(elem, scopes, flags)
            );
        case "RestElement":
